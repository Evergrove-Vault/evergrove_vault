...
Однако возникают ситуации когда простого набора команд становится недостаточно и необходимо решать такие проблемы, как слияние веток, откат коммитов, создание тегов и очистка истории.
...
**rebase** - позволяет переместить коммиты из одной ветки в цепочку коммитов другой ветки так, будто они изначально там находились. Совет. В случае использования rebase в проекте вместо merge дописывайте в название коммита название ветки, так потом вам будет намного проще понять откуда пришел данный коммит
...
**merge** - слияние двух веток в одну с созданием коммита слияния 
**worktree** - позволяет работать над несколькими ветками проекта одновременно, что сильно упрощает ситуацию когда вы не доделали изменения в какой-то ветки, но вынуждены переключиться на другую
